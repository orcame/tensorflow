...
// This file has three implementation of RESIZE_NEAREST_NEIGHBOR.
enum KernelType {
  kReference,
...
#define TF_LITE_RESIZE_NEAREST_NEIGHBOR(type, datatype)                                \
  type::ResizeBilinear(GetTensorData<datatype>(input), GetTensorDims(input),   \
    ...
    if (kernel_type == kReference) {
      TF_LITE_RESIZE_NEAREST_NEIGHBOR(reference_ops, float);
    }
    if (kernel_type == kGenericOptimized || kernel_type == kNeonOptimized) {
      TF_LITE_RESIZE_NEAREST_NEIGHBOR(optimized_ops, float);
    }
  } else if (output->type == kTfLiteUInt8) {
    if (kernel_type == kReference) {
      TF_LITE_RESIZE_NEAREST_NEIGHBOR(reference_ops, uint8_t);
    }
    if (kernel_type == kGenericOptimized || kernel_type == kNeonOptimized) {
      TF_LITE_RESIZE_NEAREST_NEIGHBOR(optimized_ops, uint8_t);
    }
#undef TF_LITE_RESIZE_NEAREST_NEIGHBOR
  } else {
...
TfLiteRegistration* Register_RESIZE_NEAREST_NEIGHBOR_GENERIC_OPT() {
  static TfLiteRegistration r = {
      nullptr, nullptr, resize_nearest_neighbor::Prepare,
      resize_nearest_neighbor::Eval<resize_nearest_neighbor::kGenericOptimized>};
  return &r;
}
